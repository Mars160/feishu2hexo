...
var hexoOpts = HexoOpts{}

func hexoDocument(ctx context.Context, client *core.Client, url string, opts *HexoOpts) error {
	...
	img_dir := fmt.Sprintf("../post_imgs/%s", sanitizeFileName)
	...
	tags := strings.Split(hexoOpts.tags, ",")
...
func hexoDocuments(ctx context.Context, client *core.Client, url string) error {
	// Validate the url to download
	...
	processFolder = func(ctx context.Context, folderPath, folderToken string) error {
		...
		opts := HexoOpts{outputDir: folderPath, dump: hexoOpts.dump, batch: false}
		for _, file := range files {
			if file.Type == "folder" {
				...
				wg.Add(1)
				go func(_url string) {
					if err := hexoDocument(ctx, client, _url, &opts); err != nil {
						errChan <- err
					}
					wg.Done()
				}(file.URL)
	...
	if err := processFolder(ctx, hexoOpts.outputDir, folderToken); err != nil {
		return err
	}
...
func hexoWiki(ctx context.Context, client *core.Client, url string) error {
	...
	downloadWikiNode = func(ctx context.Context,
		...
		for _, n := range nodes {
			...
			if n.ObjType == "docx" {
				...
				// hexoDocument(ctx, client, prefixURL+"/wiki/"+n.NodeToken, &opts)
			}
		}
		return nil
	}
...
func handleHexoCommand(url string) error {
	// Load config
	...
	if hexoOpts.batch {
		return hexoDocuments(ctx, client, url)
	}

	if hexoOpts.wiki {
		return hexoWiki(ctx, client, url)
	}

	return hexoDocument(ctx, client, url, &hexoOpts)
}
